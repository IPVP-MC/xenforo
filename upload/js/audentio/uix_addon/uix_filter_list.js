...
!function($, window, document, _undefined)
{
    ...
    XenForo.FilterList.prototype =
    {
        ...
        registerListItems: function()
        {
            ...
            this.$groups = this.$list.find('div:not(.listItem):not(.Popup):not(.Menu)');
        },
        ...
        filter: function(e)
        {
            ...
            this.$groups.each(function(i, group)
            {
                ...
                if ($group.find('.listItem:visible').length == 0)
                {
                    $group.hide();
                }
                else
    ...
    XenForo.FilterListItem.prototype =
    {
        ...
        filter: function(filterRegex)
        {
            ...
            {
                this.$textContainer.html('<span class="uix_nodeTreeHandle ui-sortable-handle"> </span>' + this.text.replace(filterRegex, '<strong>$1</strong>'));
    ...
    XenForo.Sortable = function($container)
    {
        ...
        }).bind('sortupdate', function(e)
        {
            ...
            {               
                ...
                {
                    $el.find('.noResults').show();
                }
                else
                {
                    $el.find('.noResults').hide();
                }
            });
